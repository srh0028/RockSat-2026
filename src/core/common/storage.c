...
//Fix for compiler issues with _mkdir function
#include <string.h>
...
void create_folder(char *name_ptr)
{
    ...
    int status_code = MKDIR(folder_name);
...
int store_csv(char *folder_name_ptr, csv_t *csv_ptr)
{
    ...
    for (int row = 0; row < csv_ptr->rows_int; row++)
    {
        ...
        for (int column = 0; column < csv_ptr->columns_int; column++)
        {
            ...
            if (column < (csv_ptr->columns_int - 1))
                fprintf(file, ",");
        }
...
#define LINE_COUNTING_BUFFER_SIZE 100
    ...
    while (fgets(buffer, LINE_COUNTING_BUFFER_SIZE, file) != NULL)
        how_many_lines++;
...
void replace_linear_flight_profile(void)
{
    ...
    for (i = 0; i < halfway; i++)
    {
        ...
        if ((i % (TEST_FLIGHTS_DURATION_SECONDS / TIMED_EVENT_COUNT) == 0) && (i != 0))
            profile[(i * FLIGHT_PROFILE_COLUMNS) + 2] = timed_events++;
        else
            profile[(i * FLIGHT_PROFILE_COLUMNS) + 2] = NO_TIMED_EVENT_DOUBLE;
    }
    for (i; i < TEST_FLIGHTS_DURATION_SECONDS; i++)
    {
        ...
        profile[(i * FLIGHT_PROFILE_COLUMNS) + 0] = i;
        profile[(i * FLIGHT_PROFILE_COLUMNS) + 1] = buffer;
        if (i % (TEST_FLIGHTS_DURATION_SECONDS / TIMED_EVENT_COUNT) == 0)
            profile[(i * FLIGHT_PROFILE_COLUMNS) + 2] = timed_events++;
        else
            profile[(i * FLIGHT_PROFILE_COLUMNS) + 2] = NO_TIMED_EVENT_DOUBLE;
    }

...
void simulate(void)
{
    ...
    if (simulation.status_e != WAITING_EV)
        strcpy(error_message, "Simulation is not in the \"waiting\" state.");
    if (!simulation.flight_profile_ptr)
        strcpy(error_message, "Simulation has no flight profile.");
    if (simulation.current_timed_event != TIMED_EVENT_COUNT)
        strcpy(error_message, "Simulation attempting to start after an in-sim timed event has occured.");
    if (simulation.current_tick_int != 0)
        strcpy(error_message, "Simulation attempting to start after in-sim time has already passed.");
    if (simulation.current_altitude_dbl != 0.0)
        strcpy(error_message, "Simulation attempting to start while in-sim altitude is not 0.");

...
int validate_flight_profile(csv_t *flight_profile_ptr)
{
    ...
    if (!flight_profile_ptr->data_ptr)
        return -1;
    if (!flight_profile_ptr->file_name_ptr)
        return -2;
    if (flight_profile_ptr->rows_int < 1)
        return -3;
    if (flight_profile_ptr->columns_int != 3)
        return -4;
    ...
    for (int i = 0; i < flight_profile_ptr->rows_int; i++)
    {
        ...
        if ((current_row_ptr[0] - current_tick_dbl) != 1.0)
            return -5; // next tick should be 1 more than current tick
        else
            current_tick_dbl++;

        // validate second column
        if (current_row_ptr[1] < 0)
            return -6; // altitude should never go below 0

        // validate third column
        third_row_dbl = current_row_ptr[2];
        if (third_row_dbl == NO_TIMED_EVENT_DOUBLE)
            continue; // skip "no timed event" rows
        if (third_row_dbl > TIMED_EVENT_COUNT)
            return -7; // max number of timed events
        if (third_row_dbl < 0)
            return -7; // timed event should never be negative
        if ((third_row_dbl - current_timed_event_dbl++) != 1.0)
            return -7; // timed event should be 1 more than previous timed event
    }
...
int begin_simulation()
{
    ...
    default:
        //added return statement to handle compiler error
        return 1;
    }

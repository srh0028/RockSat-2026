...
double instrument_reading_2(void) {
    ...
    if ( ( ( int ) simulation.current_timed_event ) < TIMED_EVENT_3_EV ) return INSTRUMENT_MOUNT_UNFINISHED;
    
    //Find the highest altitude reached so far and return it
    double altitude_dbl = simulation.current_altitude_dbl;
    if ( altitude_dbl > apogee_dbl ) apogee_dbl = altitude_dbl;
    return apogee_dbl;
}
...
void reset_instrument_2(void) {
    ...
    for ( int i = 0; i < PIN_COUNT; i ++ ) instrument_2.pins[ i ] = 0.0;
